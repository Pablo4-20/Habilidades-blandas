...
import { 
    ...
    ComputerDesktopIcon, ChevronLeftIcon, ChevronRightIcon
...
const AsignarMaterias = () => {
    ...
    const ITEMS_PER_PAGE = 5; // Muestra 5 docentes por página
    ...
    const getFilteredAndSortedData = () => {
        ...
        let listaVisual = Object.values(docentesAgrupados);
        
        // Ordenar alfabéticamente por Apellidos del Docente
        ...
        <div className="space-y-6 animate-fade-in flex flex-col h-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                        ...
                        <div className="md:col-span-3">
                            <CustomSelect 
                                ...
                                disabled={!periodoActivo || carreras.length === 1} 
                            />
                        </div>
                        <div className="md:col-span-2">
            ...
            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input type="text" className="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-xl bg-white text-sm outline-none focus:ring-2 focus:ring-blue-100" 
                        placeholder="Buscar por docente o materia..." 
                        value={busqueda} 
                        onChange={(e) => { setBusqueda(e.target.value); setCurrentPage(1); }} // Resetear página al buscar
                    />
                </div>
                <div className="flex gap-2 overflow-x-auto pb-1 md:pb-0">
                    <select className="h-[42px] pl-3 pr-8 rounded-xl border border-gray-300 bg-white text-sm focus:ring-2 focus:ring-blue-100 outline-none" 
                        value={filtroCiclo} 
                        onChange={(e) => { setFiltroCiclo(e.target.value); setCurrentPage(1); }}
                    >
                ...
                {(busqueda || filtroCiclo || filtroCarrera || filtroPeriodo) && (
                    <button onClick={() => { setBusqueda(''); setFiltroCiclo(''); setFiltroCarrera(''); setFiltroPeriodo(''); setCurrentPage(1); }} className="px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-xl font-medium transition border border-red-200 bg-white whitespace-nowrap">
                        Limpiar
                    </button>
                )}
            </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody className="bg-white divide-y divide-gray-50">
                            {loading ? <tr><td colSpan="4" className="text-center py-10 text-gray-400">Cargando...</td></tr> : 
                             currentItems.length === 0 ? <tr><td colSpan="4" className="text-center py-12 text-gray-400"><FunnelIcon className="h-10 w-10 mx-auto mb-2 opacity-20"/> Sin resultados.</td></tr> :
